...
my $PREFIXES = [qw(Mr. Mrs. Ms. Miss Dr.)];
my $SUFFIXES = [qw(Jr. Sr. I II III IV V MD DDS PhD DVM)];

sub FIRST_NAMES { $FIRST_NAMES }
sub LAST_NAMES  { $LAST_NAMES  }
sub PREFIXES    { $PREFIXES    }
sub SUFFIXES    { $SUFFIXES    }

sub prefix {
    my ($class) = @_;
    random($class->PREFIXES());
}

sub suffix {
    my ($class) = @_;
    random($class->SUFFIXES());
}

sub first_name {
    my ($class) = @_;
    random($class->FIRST_NAMES);
}

sub last_name  {
    my ($class) = @_;
    random($class->LAST_NAMES);
}

sub name {
    my ($class) = @_;
    my $first_name = $class->first_name;
    my $last_name  = $class->last_name;
